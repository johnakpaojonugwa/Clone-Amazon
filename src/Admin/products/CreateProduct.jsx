...
function CreateProduct() {
  ...
  const handleAddImage = () => {
    if (!formData.tempImage.trim()) {
      toast.warning("Please enter an image URL");
      return;
    }
    setFormData((prev) => ({
      ...prev,
      images: [...prev.images, prev.tempImage],
      tempImage: "",
    }));
  };

  // Remove a thumbnail image
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { title, descp, price, brand, quantity, images, category_id } =
      formData;
    ...
    if (
      ...
      toast.warning("Please fill in all fields and add at least one image");
      return;
    }
    ...
    try {
      setLoading(true);
      console.log("Creating product:", product_data);
      const res = await axios.post(`${API_BASE_URL}/products`, product_data);
      console.log("Product created:", res.data);
      toast.success("Product created successfully");
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-teal-700 text-white rounded-md hover:bg-teal-600"
                >
                  Add
                </button>
              </div>
